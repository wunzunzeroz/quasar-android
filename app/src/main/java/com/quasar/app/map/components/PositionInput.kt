...
fun LatLngDecInput(onSubmit: (Position) -> Unit, initialValue: Position?) {
    var latitude by remember {
        mutableStateOf(
            initialValue?.latLngDecimal?.latitude?.toString() ?: ""
        )
    }
    var longitude by remember {
        mutableStateOf(
            initialValue?.latLngDecimal?.longitude?.toString() ?: ""
        )
    }
...
fun LatLngDdmInput(onSubmit: (Position) -> Unit, initialValue: Position?) {
    var latDeg by remember {
        mutableStateOf(
            initialValue?.latLngDegreesMinutes?.latitude?.degrees?.toString()
                ?: ""
        )
    }
    var latMin by remember {
        mutableStateOf(
            initialValue?.latLngDegreesMinutes?.latitude?.minutes?.toString() ?: ""
        )
    }
    var latDir by remember { mutableStateOf(CardinalDirection.S) }

    var lngDeg by remember {
        mutableStateOf(
            initialValue?.latLngDegreesMinutes?.longitude?.degrees?.toString() ?: ""
        )
    }
    var lngMin by remember {
        mutableStateOf(
            initialValue?.latLngDegreesMinutes?.longitude?.minutes?.toString() ?: ""
        )
    }
